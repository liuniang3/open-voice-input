...
会议实时转录使用独立于短语音输入的会话、原生录音和持久化状态。Windows 使用 WASAPI，macOS 使用 AVAudioEngine 与 ScreenCaptureKit（macOS 13+）。当前流程为：阿里流式识别、停止后可选 MiMo 全音频复核、独立 LLM 校订、单独生成详细总结与思维导图。

本文描述当前开发版。旧版每约 30 秒请求一次 MiMo/Qwen 文件接口的方式不是当前会议实时默认流程；30 秒仍是 Markdown 自动保存周期。

## 模型和凭证

| 环节 | 当前模型 / 接口 | 何时调用 |
| --- | --- | --- |
| 实时识别 | 默认 `qwen-audio-3.0-asr-flash-streaming`，可选 `fun-asr-realtime` | 录制时发送已落盘音频；停止后可重试缺口 |
...
`qwen3-asr-flash-realtime` 使用不同的 `/realtime` 协议，不能当作新 Streaming 模型使用；普通 `fun-asr` 文件接口也不是 `fun-asr-realtime`。当前会议流式传输只接受表中两个准确的模型 ID。旧批处理模型配置保留，新实时模型需要配置自己的凭证。

MiMo 复核使用普通 API 地址与普通 API Key，不接受 Token Plan。阿里实时识别、MiMo ASR 复核和 LLM 各按所选模型解析配置，不跨模型借用 URL 或 Key。不开启复核时不会要求 MiMo 凭证。
...
1. 在会议模型设置中为所选阿里 Streaming 或 Fun-ASR 实时模型填写 API 地址和 Key。会后需要复核或校订时，再分别配置 MiMo ASR 和 LLM。
2. 打开“会议实时转录”，选择仅系统声音、仅麦克风或双轨，可提前选择 Markdown 文件路径。
3. 点击开始，或按设置中的会议快捷键。再次按快捷键只置前当前会话。
4. 录音先写入本地，再以有界 PCM 数据包发送到 WebSocket。实时草稿可以被后续草稿替换；只有确认句子进入已确认原文，按时间顺序保留。草稿不是最终识别结果。
5. 录制时可切换悬浮窗、精简视图和置顶，再返回详情；窗口切换不新建录音、不触发模型处理。暂停/继续保留同一会话，也可在暂停时停止。
6. Markdown 每 30 秒独立保存，暂停和停止时也会保存。网络请求不会阻塞原生音频落盘，自动保存不以 ASR 返回为前提。
7. 停止后等待原生尾段封存、WAV 收尾和流式任务结束确认。超时或连接失败会保留缺口供重试，不能当作全部转写完成。停止本身不触发 MiMo 或 LLM。
...
实时发送按节奏从完整归档读取有界音频，连接窗口轮换不限制会议总长度。断线可重新连接处理后续内容；未完成的窗口、未发送区间及已确认句子保留检查点。失败窗口不会因为后续连接成功而消失，停止后可以明确重试。重试按窗口身份处理，不能仅按文本相同就删除真实重复发言。
...
原生采集、转写网络请求、30 秒 Markdown 自动保存相互独立。ASR 网络故障不应停止录音；磁盘写入失败或设备故障仍可能使原生采集停止，此时应保留已提交音频并恢复，不能继续宣称录制正常。
